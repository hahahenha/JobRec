...
log = spark.read.parquet(f'{file_path}/0data/202code2_query_{city_short}/log')
...
# For training data
person_train = spark.read.parquet(f'{file_path}/train/person')\
...
# For testing data
person_test = spark.read.parquet(f'{file_path}/test/person')\
                        .select(F.col('geek_id').alias('geekID'),F.col('prediction').alias('pred_person'),F.col('date').alias('date_person'))
person_test.show(1)
...
# For training data
occ_train = spark.read.parquet(f'{file_path}/train/occ')\
...
# For testing data
occ_test = spark.read.parquet(f'{file_path}/test/occ')\
                    .select(F.col('job_id').alias('jobID'),F.col('prediction').alias('pred_job'),F.col('date').alias('date_job'))
occ_test.show(1)
...
log_train.write.mode('overwrite').parquet(f'{file_path}/train/log')
log_test.write.mode('overwrite').parquet(f'{file_path}/test/log')
print('log train cnt:', log_train.count())
print('log test cnt:', log_test.count())
