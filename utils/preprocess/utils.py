...
r"""
The :mod:`gsp.utils` module implements some utility functions used throughout
the package.
"""
...
def loadmat(path):
    ...
    Examples
    --------
    >>> from utils.preprocess import utils
    >>> data = utils.loadmat('pointclouds/bunny')
    >>> data['bunny'].shape
    (2503, 3)

    """
    data = pkgutil.get_data('gsp', 'data/' + path + '.mat')
...
def import_classes(names, src, dst):
    ...
    for name in names:
        module = importlib.import_module('utils.preprocess.' + src + '.' + name.lower())
        setattr(sys.modules['utils.preprocess.' + dst], name, getattr(module, name))


def import_functions(names, src, dst):
    """Import functions in package from their implementation modules."""
    for name in names:
        module = importlib.import_module('utils.preprocess.' + src)
        setattr(sys.modules['utils.preprocess.' + dst], name, getattr(module, name))
