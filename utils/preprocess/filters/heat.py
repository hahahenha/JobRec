...
from utils.preprocess.filters.filter import Filter  # prevent circular import in Python < 3.5
...
class Heat(Filter):
    ...
    def __init__(self, G, tau=10, normalize=False, **kwargs):
        ...
        def kernel(x, t):
            ...
            # if t < 0:
            #     return (x+1.0) / (G.lmax+1.0) * np.exp(-t * (x+1.0) / (G.lmax+1.0))
            # else:
            #     return (G.lmax+1.0) / (x + 1.0) * np.exp(-t * (x+1.0) / (G.lmax+1.0))
